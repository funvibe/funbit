...
// TestBuilderOverflowError tests overflow during construction
func TestBuilderOverflowError(t *testing.T) {
	// Test integer overflow - 256 doesn't fit in 8 bits
	_, err := builder.NewBuilder().
		AddInteger(256, bitstringpkg.WithSize(8)). // 256 > 2^8-1 = 255
		Build()
	...
	var bitstringErr *bitstringpkg.BitStringError
	if errors.As(err, &bitstringErr) {
		if bitstringErr.Code != "OVERFLOW" {
			t.Errorf("Expected OVERFLOW error, got %s", bitstringErr.Code)
		}
	} else {
		t.Errorf("Expected BitStringError, got %T", err)
	}

	// Test that overflow value is truncated (as per Erlang behavior)
	bs, err := builder.NewBuilder().
		AddInteger(256, bitstringpkg.WithSize(8)). // Should truncate to 0
		Build()

	// The current implementation might succeed with truncation, which is Erlang behavior
	if err == nil {
		var value int
		_, err := matcher.NewMatcher().
		...
		if value != 0 { // 256 % 256 = 0
			t.Errorf("Expected truncated value 0, got %d", value)
		}
	}

	// Test negative overflow for signed integers
	_, err = builder.NewBuilder().
		AddInteger(-129, bitstringpkg.WithSize(8), bitstringpkg.WithSigned(true)). // -129 < -128
		Build()

	if err == nil {
		t.Fatal("Expected signed overflow error, got nil")
	}

	var bitstringErr2 *bitstringpkg.BitStringError
	if errors.As(err, &bitstringErr2) {
		if bitstringErr2.Code != "SIGNED_OVERFLOW" {
			t.Errorf("Expected SIGNED_OVERFLOW error, got %s", bitstringErr2.Code)
		}
	}
}
...
func TestUTFErrors(t *testing.T) {
	...
	_, err := builder.NewBuilder().
		AddSegment(bitstringpkg.Segment{
			Value: "Hello",
			Size:  40, // Size should not be specified for UTF
			Type:  "utf8",
		}).
		Build()

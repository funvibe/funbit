...
// TestBitstringTypes_IntegerType тестирование явного указания типа integer
...
// TestBitstringTypes_Float32 тестирование 32-битных float значений
func TestBitstringTypes_Float32(t *testing.T) {
	...
	// Для float сравнения используем допустимую погрешность
	if math.Abs(float64(result-3.14)) > 0.001 {
		t.Errorf("Expected float ~3.14, got %f", result)
	}
}

// TestBitstringTypes_Float64 тестирование 64-битных float значений
...
// TestBitstringTypes_Binary тестирование binary типа (выравнено по байтам)
...
// TestBitstringTypes_CombinedBinary тестирование объединения binary данных
func TestBitstringTypes_CombinedBinary(t *testing.T) {
	// <<data/binary, " world"/binary>> где data = <<"hello">>
	data := "hello"
	world := " world"
	...
	var result []byte
	_, err = matcher.NewMatcher().
		Binary(&result, bitstring.WithSize(11), bitstring.WithType("binary")). // 5 + 6 = 11 байт
		Match(bs)
...
func TestBitstringTypes_Bitstring(t *testing.T) {
	...
	// Должно получиться 8 бит (1 байт) с значением 10110000 в двоичном виде = 0xB0
	if bs.Length() != 8 {
...
// TestBitstringTypes_FloatSpecialValues тестирование специальных float значений
func TestBitstringTypes_FloatSpecialValues(t *testing.T) {
	// Тестируем NaN, Infinity, -Infinity
	testCases := []struct {
		name    string
		value   float64
		size    uint
		isNaN   bool
		isInf   bool
		infSign int // 1 для +Inf, -1 для -Inf
	}{
...
// TestBitstringTypes_BinaryAlignment тестирование выравнивания binary данных
func TestBitstringTypes_BinaryAlignment(t *testing.T) {
	// Тест: бинарные данные должны быть выровнены по байтам
	bs, err := builder.NewBuilder().
		AddInteger(1, bitstring.WithSize(4)).                  // 4 бита
		AddBinary([]byte("AB"), bitstring.WithType("binary")). // 16 бит (2 байта)
		AddInteger(2, bitstring.WithSize(4)).                  // 4 бита
		Build()
	...
	// Общая длина: 4 + 16 + 4 = 24 бита = 3 байта
	if bs.Length() != 24 {
...
func TestBitstringTypes_BitstringAlignment(t *testing.T) {
	...
	// Общая длина: 3 + 5 + 7 + 1 = 16 бит = 2 байта
	if bs.Length() != 16 {
